...
class FluxKontextAlgorithm(QgsProcessingAlgorithm):
    ...
    def displayName(self):
        return "FLUX Kontext Image Editing"
    ...
    def shortHelpString(self):
        return """
Transform map tiles using FLUX.1 Kontext [pro] Image Editing.

FLUX.1 Kontext [pro] Features:
• Simple image editing with text prompts
• Character consistency across edits  
• Automatic dimension matching
• Optimized for object modifications

Parameters:
- Input Raster: Source raster layer (any layer - Canvas view will be used)
- API Key: Your FLUX API key from https://api.bfl.ai
- Edit Prompt: What to edit/change (e.g. "Change to watercolor style", "Make buildings red")
- Tile Size: Size of each tile in pixels (512, 1024)
- Output Directory: Folder to save edited tiles
- Format: PNG (with transparency) or JPEG
- Safety: Content moderation (0-6, higher = more permissive)
- Seed: Optional random seed for reproducible results
- Create VRT: Build a Virtual Raster mosaic of all tiles

The algorithm renders your current map view, sends it to FLUX.1 Kontext [pro] for AI editing,
and downloads the results with proper world files for georeferencing.
        """

    def initAlgorithm(self, config=None):
        # Input raster layer (wird ignoriert, Canvas wird verwendet)
        self.addParameter(
            QgsProcessingParameterRasterLayer(
                self.INPUT_RASTER,
                "Input Raster Layer (Canvas view will be used)",
                optional=False
            )
        )
        ...
        # Edit prompt - einfacher für Kontext
        self.addParameter(
            QgsProcessingParameterString(
                self.PROMPT,
                "Edit Prompt (what to change)",
                defaultValue="Change to watercolor painting style",
                multiLine=True,
                optional=False
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                ...
                defaultValue=1,  # Default 1024 für bessere Qualität
                optional=False
            )
        )
    ...
    def processAlgorithm(self, parameters, context: QgsProcessingContext, feedback: QgsProcessingFeedback):
        ...
        safety = self.parameterAsInt(parameters, self.SAFETY, context) if parameters.get(self.SAFETY) is not None else 2
        ...
        # Validate inputs
        if not api_key:
            raise QgsProcessingException("API Key ist erforderlich. Holen Sie sich einen auf https://api.bfl.ai")
        if not prompt:
            raise QgsProcessingException("Edit Prompt ist erforderlich")

        # Convert parameters
        N = 512 if tile_size_idx == 0 else 1024
        image_format = "PNG" if format_idx == 0 else "JPEG"

        feedback.pushInfo(f"🎨 Starte FLUX Kontext Image Editing...")
        feedback.pushInfo(f"Eingabe: {raster_layer.name()}")
        feedback.pushInfo(f"Tile-Größe: {N}×{N}")
        feedback.pushInfo(f"Format: {image_format}")
        feedback.pushInfo(f"Edit-Prompt: {prompt[:80]}...")
        feedback.pushInfo(f"Ausgabe: {output_dir}")

        # Initialize FLUX processor mit Kontext endpoint
        log_path = os.path.join(output_dir, "flux_kontext_processing.log")
        ...
        try:
            ...
            # Generate filename
            filename = f"kontext_tile_{row:03d}_{col:03d}.{image_format.lower()}"
            ...
            # ✅ FLUX KONTEXT PARAMETERS (einfacher als Ultra!)
            kontext_params = {
                "safety_tolerance": safety,
                "use_kontext_api": True  # Flag für unsere FluxStylizeTiles Klasse
            }

            # Process tile mit Kontext API
            result = flux.process_tile(
                ...
                seed=seed,
                image_format=image_format,
                payload=kontext_params
            )
            
            if result["status"] == "Ready":
                tile_files.append(output_path)
                feedback.pushInfo(f"✓ Tile erfolgreich editiert: {filename}")
                
                # ✅ AUTOMATISCHES LADEN ALS RASTER-LAYER
                try:
                    layer_name = f"FLUX Kontext - {raster_layer.name()}"
            ...
            if create_vrt and tile_files:
                try:
                    vrt_path = os.path.join(output_dir, "kontext_stylized_mosaic.vrt")
                    flux.build_vrt(vrt_path, tile_files)
                    
                    # VRT auch automatisch laden
                    vrt_layer = QgsRasterLayer(vrt_path, "FLUX Kontext VRT Mosaic", "gdal")
                    ...
                    feedback.pushInfo(f"✓ VRT Mosaik erstellt und geladen: kontext_stylized_mosaic.vrt")
                except RuntimeError as e:
                    feedback.pushWarning(f"VRT Erstellung übersprungen: {e}")

            feedback.pushInfo(f"🎉 Kontext Editing abgeschlossen. {len(tile_files)} Tiles erstellt und geladen.")
            ...
            raise QgsProcessingException(f"Fehler bei FLUX Kontext Verarbeitung: {str(e)}")
